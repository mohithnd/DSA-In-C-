...
void collect_leaves(Node *root, vector<int> &res)
{
    if (root == nullptr)
    {
        return;
    ...
    collect_leaves(root->left, res);
    collect_leaves(root->right, res);
}
...
{
    vector<int> res;
    collect_left_boundary(root, res);
    collect_leaves(root, res);
    collect_right_boundary(root->right, res);
    for (int i : res)
    {
        cout << i << " ";
